...
import { getEventDetailsFromId } from "@/utils/getEventDetailsFromId";
import { getUser, getUserProfile } from "@/utils/UserFunctions";
import GetUser from "@/hooks/getUser";

const ParticipationDetails = dynamic(
  () => import("@/components/Modals/ParticipationDetails"),
  {
    loading: () => <></>,
    ssr: false,
  }
            ...
            if (profile[0].role === "participant") router.replace("/");
              ...
              if (profile[0].role === "superadmin") {
                getEvents("id,name").then((events) =>
                  setEvents(
                    events as unknown as Database["public"]["Tables"]["events"]["Row"][]
                  )
                );
